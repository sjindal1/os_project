...
uint16_t read(hba_port_t *port, uint32_t startl, uint32_t starth, uint32_t count, uint16_t *buf)
{
  int i;
  port->ie = 0xffffffff;    // Clear pending interrupt bits
  ...
  while (1)
  {
    ...
    if ((port->ci & (1 << slot)) == 0) 
      break;
    if (port->ie & HBA_PxIS_TFES)  // Task file error
    {
